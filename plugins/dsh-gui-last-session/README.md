...
```sh
# from the directory that contains `plugins/dsh-gui-last-session`
dsh plugin --profile web add file:./plugins/dsh-gui-last-session
```

Or via the DSH GUI's bundled engine bin:

```sh
node "<engine>/lib/bin.js" plugin --profile web add file:./plugins/dsh-gui-last-session
```

The engine's `plugin` command is a thin pnpm forwarder: a local `file:` spec is
installed by its **true package name** and, because the manifest declares
`dsh.bundle.patch`, it joins the profile's bundle layer stack automatically.
Bundle layers are read at boot, so **restart the engine** afterwards.
