...
> **This route is protected by the engine's trust fence** (Host allow-list + browser
> session cookie). It returns account usage and balance, so **a bare `curl` without the
> cookie gets `401 unauthorized`** (before the fix it returned 200 — any local process,
> and any page subject to DNS rebinding, could read it). Same-origin page requests carry
> the cookie automatically; calling it by hand means first exchanging one session cookie
> for the URL printed by `dsh web`, then passing it as `-H 'cookie: …'` (see
> `dsh-gui-last-session/README.md` for an example).
...
>
> The plan was to publish the four bundled plugins to npm, so that `dsh plugin add dsh-model-surplus` would
...
>
> Publishing to npm will resume once sign-up works; `dsh plugin --profile web add dsh-model-surplus` will work
> then.

**Option 1 — DSH Ready GUI (recommended)**: these four plugins ship inside the GUI. Open the GUI →
Settings → Third-party plugins → tick **模型余量（dsh-model-surplus）**. Install, uninstall and enable/disable
